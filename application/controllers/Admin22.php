<?php
class Admin22 extends CI_Controller
{
	...
	public function index()
	{
		...
		if (!@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Admin | BBMK 2022";
			$this->load->view("bbmk22/template/header", $data);
			$this->load->view("bbmk22/ukm/login");
			$this->load->view("bbmk22/home/footer");
		} else {
			$this->alert->msg("success", "Anda sudah login...", "Session anda diaktifkan", 1, base_url("/admin22/home"));
		}
	}
	...
	public function home($id = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("bbmk22/ukm/peserta");
	...
	public function edit()
	{
		...
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("bbmk22/ukm/header");
			$this->load->view("bbmk22/ukm/edit");
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function notif()
	{
		...
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/notif");
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function profile()
	{
		...
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/profile");
			$this->load->view("bbmk22/ukm/footer");
		} else {
	...
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Daftar Peserta | BBMK 2022";
			if ($status == null) {
				$status = "semua";
			}
			$data["status"] = $status;
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/peserta", $data);
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function materi($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			// $this->load->view("head", $data);
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/materi");
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Kehadiran Peserta | BBMK 2022";
			if ($status == null) {
				$status = "semua";
			}
			$data["status"] = $status;
			// $this->load->view("head", $data);
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/kehadiran");
			$this->load->view("bbmk22/ukm/footer");
		} else {
	...
	public function daftarhadir($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			// $this->load->view("head", $data);
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/daftarhadir");
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function statistik($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			// $this->load->view("head", $data);
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/statistik");
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Kelulusan Peserta | BBMK 2022";
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/kelulusan");
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Sertifikat Peserta | BBMK 2022";
			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/sertifikat");
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Profile Peserta | BBMK 2022";
			$data["pid"] = $pid;

			$this->load->view("bbmk22/ukm/header", $data);
			$this->load->view("bbmk22/ukm/datapeserta", $data);
			$this->load->view("bbmk22/ukm/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	{
		session_destroy();
		$this->alert->msg("success", "Logout berhasil...", "", 1, base_url("/admin22"));
	}
